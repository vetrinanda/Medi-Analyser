...
class Agent:
    def __init__(self,medical_report=None,role=None,extra=None):
    ...
    def create_prompt(self):
        if self.role=="MultidisciplinaryTeam":
            ...
            return ChatPromptTemplate.from_template(selected_templates)

    def run(self):
        if self.medical_report:
            prompt = self.prompt.format(medical_report=self.medical_report)
        else:
...
class MultidisciplinaryTeam(Agent):
    def __init__(self, cardiologist_report, psychologist_report, pulmonologist_report):
        ...
        super().__init__(role = "MultidisciplinaryTeam", extra=extra_info)
